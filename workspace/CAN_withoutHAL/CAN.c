...
void CAN_setup (uint32_t ctrl)  {
  ...
  pCAN->MCR = (CAN_MCR_INRQ   |           /* initialisation request           */
               CAN_MCR_NART |							/* no automatic retransmission      */
							 CAN_MCR_ABOM						    /* automatically recover from Bus-Off*/
						 //| CAN_MCR_TTCM	            /* Time triggered communication mode (Necessary to use the timestamps)*/
							 );         								/* CAN_MCR_RFLM should be cleared*/
...
void CAN_rdMsg (uint32_t ctrl, CAN_msg *msg)  {
  ...
  pCAN->RF0R |= CAN_RF0R_RFOM0;             /* Release FIFO 0 output mailbox */
	printf ("\nReceived: %d", msg->data[0]);
	//printf ("\nReceived: %d at %u", msg->data[0],pCAN->sFIFOMailBox[0].RDTR&CAN_RDT0R_TIME);
	Delay (100);
}
